function Level() {
    ...
    this.canPlayerMoveTo = function (x, y) {
        var flag = x < 0 || y < 0 || x >= this.width || y >= this.height;
        if (!flag) {
            this.errorMessage = this.cells[x][y].isWall();
            return !this.errorMessage;
        } else {
            this.errorMessage = Cell.WALL_CELL.isWall();
        }
        return false;
    };
        ...
        var execFunction = (function (level) {
            ...
            return function () {
                if (level.player.isReady()) {
                    ...
                    setTimeout(function(){
                        level.cells[level.player.reqX][level.player.reqY].stepIn(level.player);
                    }, 0);
                    ...
                    if (!action) {
                        ...
                        if (level.player.reqX != level.player.curX || level.player.reqY != level.player.curY) {
                            setTimeout(function(){
                                level.cells[level.player.curX][level.player.curY].stepOut(level.player);
                            }, 0);
                        }
