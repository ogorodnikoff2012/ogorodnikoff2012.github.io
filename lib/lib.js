...
function updateLevelIndicator(isGlobal) {
    ...
    if (!isGlobal || levelManager.getNextLevel()) {
        levelIndicator.innerText = i18n.translate("Level:") + " " + (isGlobal ? levelManager.getNextLevel() : level.levelName);
    }
}
...
function updateMoneyIndicator(isGlobal) {
    ...
    if (!isGlobal || localStorage["coins"]) {
        moneyIndicator.innerText = i18n.translate("Money:") + " " + (isGlobal ? localStorage["coins"] : level.collectedCoins);
    }
}